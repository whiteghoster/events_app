...
export function useEventContractorsDialog({
  ...
  const validate = (): string | null => {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!e.contractorId) {
        return `${label}: select a contractor`
      }
      if (e.shift === 'none') {
        return `${label}: select a shift`
      }
      if (!e.memberQuantity || e.memberQuantity <= 0) {
        return `${label}: quantity must be greater than 0`
      }
      if (!e.workDate) {
        return `${label}: set a work date`
      }
      if (worksFrom && e.workDate < worksFrom) {
        return `${label}: work date is before Works From`
      }
      if (worksTo && e.workDate > worksTo) {
        return `${label}: work date is after Works To`
      }
      const uniqueKey =
