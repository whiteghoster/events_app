...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002' // ✅ Updated
  ...
  const login = useCallback(
    async (email: string, password: string): Promise<boolean> => {
      ...
      try {
        ...
        // Store user - use lowercase role!
        const userData: User = {
          id: data.user.id,
          name: data.user.name || data.user.email,
          email: data.user.email,
          role: (data.user.role || 'staff').toLowerCase() as UserRole, // ✅ Ensure lowercase
